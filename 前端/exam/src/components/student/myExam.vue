  ...
  <div id="myExam">
    ...
    <div class="wrapper">
      ...
      <ul class="top">
        ...
        <input type="text" placeholder="试卷名称" class="search" v-model="key" @keyup.enter="search()">
        <i class="el-icon-search"></i></div></li>
...
export default {
  ...
  methods: {
    //获取当前所有考试信息
    getExamInfo() {
      this.$axios(`/api/partexams/${this.pagination.current}/${this.pagination.size}`).then(res => {
    ...
    //搜索试卷
    search() {
      this.$axios('/api/partexams').then(res => {
