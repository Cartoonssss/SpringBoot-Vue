    ...
    <div class="wrapper">
      ...
      <div class="all">
        <ul class="msglist">
          ...
          >
						<p class="title"> <i class="el-icon-s-custom"></i>{{data.name}}</p>
            ...
            <div v-for="(replayData,index2) in data.replays" :key="index2">
              <p class="comment"><i class="el-icon-s-custom"></i>{{replayData.replayName}}<i class="iconfont icon-huifuxiaoxi"></i>{{replayData.replay}}</p>
            </div>
  ...
  methods: {
    ...
    submit() {
      ...
      this.$axios({
        ...
        method: "post",
        data: {
					name: this.$cookies.get("studentName"),
          title: this.title,
      ...
      this.$prompt('回复留言', '提示', {
        ...
        this.$axios({
          ...
          data: {
            ...
            replayTime: date,
            messageId: messageId,
						replayName: this.$cookies.get("studentName")
          }
        }).then(res => {
          this.getMsg()
        })
